...
for file in "$BASE_DIR/music/"*.mp3; do
    ...
    if [[ -n "$artist" ]]; then
        ...
        # Sanitize filename:
        sanitized_filename=$(basename "$file" .mp3)

        # 1. Remove the artist’s name (case-insensitive) and any leading dashes
        sanitized_filename=$(echo "$sanitized_filename" | sed -E "s/^$clean_artist\s*-*\s*//I")

        # 2. Remove everything after "FT" or "ft"
        sanitized_filename=$(echo "$sanitized_filename" | sed -E 's/\s*[Ff][Tt].*//')

        # 3. Remove common YouTube branding and "(Official ...)" variations
        sanitized_filename=$(echo "$sanitized_filename" | sed -E 's/\b(VEVO|Vevo|Official|Topic|Music|Records|Entertainment|videos|Videos|official|music|MUSIC|OFFICIAL|VIDEOS|CHANNEL|Channel|channel|TV)\b//gi')

        # 4. Remove "(Official Video)", "(Official Audio)", "(Lyrics)", "(Video)", etc.
        sanitized_filename=$(echo "$sanitized_filename" | sed -E 's/\s*\(?(Official (Video|Audio|Song|Music|Track|Lyrics|Version|Remix)|Video)\)?//gi')

        # 5. Trim extra spaces and remove leading/trailing dashes or underscores
        sanitized_filename=$(echo "$sanitized_filename" | sed 's/  */ /g' | sed 's/^[-_ ]*//;s/[-_ ]*$//')
        ...
        ffmpeg -i "$new_file" -metadata artist="$clean_artist" -metadata title="$sanitized_filename" -codec copy "${new_file%.mp3}_temp.mp3"
